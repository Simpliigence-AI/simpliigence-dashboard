            ...
            <div>
              ...
              <p className="text-xs text-muted mt-1">
                {total === 0
                  ...
                  : <>Type a status update for each active requisition. <kbd className="text-[10px] bg-surface-2 border border-line px-1 rounded">Tab</kbd> or <kbd className="text-[10px] bg-surface-2 border border-line px-1 rounded">Enter</kbd> saves + advances. <kbd className="text-[10px] bg-surface-2 border border-line px-1 rounded">Esc</kbd> closes.</>
                }
              </p>
            </div>
                  ...
                  <li key={r.id} className={`px-4 py-3 flex items-center gap-3 ${isSaved ? 'bg-emerald-50/40' : isSkipped ? 'opacity-50' : ''}`}>
                    <div className="flex-shrink-0 w-6 text-center">
                      {isSaved
                        ...
                        : isSkipped
                          ? <SkipForward size={12} className="text-muted/70 inline" />
                          : <span className="text-[10px] text-muted/70 tabular-nums">{idx + 1}</span>}
                    </div>
                    ...
                    <input
                      ...
                      className="flex-1 h-8 px-3 text-xs border border-line rounded-md focus:border-violet-400 focus:outline-none focus:ring-2 focus:ring-violet-200 disabled:bg-surface-2/70 disabled:text-muted/70"
                    />
                    {!isSaved && !isSkipped && (
                      <button type="button"
                              onClick={() => skipOne(r.id, idx)}
                              className="text-[11px] text-muted/70 hover:text-ink/80 hover:bg-surface-2 px-2 py-1 rounded inline-flex items-center gap-1"
                              title="Skip — don't log a status for this req today">
                        <SkipForward size={11} /> Skip
                      </button>
                    )}
                    {isSaving && <Loader2 size={12} className="animate-spin text-muted/70" />}
                  </li>
                );
              })}
            </ul>
