  ...
  return (
    <div className="space-y-4 max-w-3xl">
      <div className="flex items-center justify-between">
        ...
        {saving && <div className="text-[11px] text-muted/70 inline-flex items-center gap-1"><Loader2 size={11} className="animate-spin" /> Saving</div>}
      </div>
          ...
          {[1, 2, 3, 4, 5].map((n) => {
            ...
            return (
              <button key={n} type="button"
                      ...
                      className={`p-1 rounded transition-colors ${filled ? 'text-amber-400' : 'text-line hover:text-amber-300'}`}
                      title={`${n} of 5`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 ml-6">
            <div>
              ...
              <input type="date" value={state.surveyAt ?? ''} onChange={(e) => patch({ surveyAt: e.target.value || null })} onBlur={blur}
                     className="w-[160px] px-2 py-1 text-xs border border-line rounded" />
            </div>
            <div>
              <label className="text-[10px] font-semibold text-muted uppercase tracking-wider mb-1 block">Score / NPS</label>
      ...
      <div className="rounded-lg border border-line bg-surface p-4">
        ...
        {state.qbrDone && (
          <div className="ml-6 space-y-2">
            ...
            <div>
              <label className="text-[10px] font-semibold text-muted uppercase tracking-wider mb-1 block">QBR notes</label>
              <textarea value={state.qbrNotes} onChange={(e) => patch({ qbrNotes: e.target.value })} onBlur={blur}
                        ...
                        className="w-full text-xs px-2 py-1.5 border border-line rounded resize-y" />
            </div>
          </div>
        )}
      </div>
