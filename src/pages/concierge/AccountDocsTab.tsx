...
export function AccountDocsTab({ accountId, mode }: Props) {
  ...
  return (
    <div className="space-y-4">
      {/* Upload zone */}
      <div className="border-2 border-dashed border-line rounded-lg p-4 bg-surface-2/50">
          ...
          <div className="mt-3 space-y-2 border-t border-line pt-3">
            <div className="flex gap-2">
              ...
              <input
                ...
                className="px-3 py-1.5 rounded border border-line text-sm"
              />
            </div>
            <textarea
              value={transText}
              ...
              rows={8}
              className="w-full px-3 py-2 rounded border border-line text-sm font-mono"
            />
    ...
    <div className="rounded-lg border border-line bg-white p-3 hover:border-sky-300 transition-colors">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <button type="button" onClick={onOpen} className="text-sm font-semibold text-ink hover:text-sky-700 text-left truncate block w-full">
            {doc.title}
          </button>
          ...
          <StatusPill doc={doc} />
          {doc.aiSummary && (
            <div className="text-xs text-ink/80 mt-1.5 leading-relaxed line-clamp-2">{doc.aiSummary}</div>
          )}
          {doc.aiError && (
        ...
        <div className="flex flex-col gap-1 items-end flex-shrink-0">
          <div className="flex gap-1">
            {doc.storagePath && (
              <button type="button" onClick={onDownload} title="Download" className="p-1 text-muted/70 hover:text-ink/80">
                <Download size={14} />
              </button>
            )}
            <button type="button" onClick={onProcess} title="Re-summarize" className="p-1 text-muted/70 hover:text-ink/80" disabled={doc.aiStatus === 'processing'}>
              <RefreshCw size={14} className={doc.aiStatus === 'processing' ? 'animate-spin' : ''} />
            </button>
            <button type="button" onClick={onRemove} title="Delete" className="p-1 text-muted/70 hover:text-rose-600">
              <Trash2 size={14} />
            </button>
          </div>
        </div>
...
function StatusPill({ doc }: { doc: AccountDocument }) {
  const map: Record<AccountDocument['aiStatus'], { label: string; cls: string; icon: JSX.Element | null }> = {
    pending: { label: 'Queued', cls: 'bg-surface-2 text-muted', icon: null },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl max-h-[85vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="px-5 py-4 border-b border-line/60 flex items-start justify-between">
          ...
          <button onClick={onClose} className="text-muted/70 hover:text-ink/80 p-1">✕</button>
        </div>
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {doc.aiSummary && (
            <section>
              <h4 className="text-[11px] font-bold text-muted uppercase tracking-wider mb-1">AI Summary</h4>
              <p className="text-sm text-ink leading-relaxed">{doc.aiSummary}</p>
            </section>
          )}
          {doc.aiTopics && (
          ...
          {doc.rawText && (
            <section>
              <h4 className="text-[11px] font-bold text-muted uppercase tracking-wider mb-1">Full text</h4>
              <pre className="text-xs bg-surface-2/70 border border-line rounded p-3 whitespace-pre-wrap font-mono max-h-96 overflow-y-auto">{doc.rawText}</pre>
            </section>
          )}
        </div>
      </div>
  ...
  return (
    <div className="space-y-3">
      {sections.map((s) => {
        ...
        return (
          <section key={s.key}>
            <h4 className="text-[11px] font-bold text-muted uppercase tracking-wider mb-1">{s.label}</h4>
            {el}
          </section>
        );
      })}
  ...
  return (
    <ul className="space-y-1.5">
      {v.map((item, i) => {
        ...
        return (
          <li key={i} className="text-xs text-ink border border-line/60 rounded px-2 py-1.5">
            {keys.map((k) => {
              const val = o[k];
              if (val == null || val === '') return null;
              return <div key={k}><span className="text-muted font-medium">{k}: </span>{String(val)}</div>;
            })}
          </li>
        );
      })}
