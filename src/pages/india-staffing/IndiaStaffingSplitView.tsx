      ...
      <Card className="col-span-12 lg:col-span-5 xl:col-span-4 p-0 overflow-hidden">
        ...
        <div className="max-h-[calc(100vh-260px)] overflow-y-auto">
          {grouped.length === 0 ? (
            ...
            <div key={account} className="border-b border-slate-100 last:border-0">
              <div className="sticky top-0 z-10 bg-gradient-to-r from-blue-50 via-indigo-50 to-violet-50 border-b border-blue-100 px-3 py-1.5 flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-900 flex-1 truncate">{account}</span>
                <span className="text-[9px] text-slate-500 font-semibold">{acctRows.length} · {acctRows.reduce((s, r) => s + r.openPositions, 0)} open</span>
              </div>
              {acctRows.map((r) => {
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="flex-1 min-w-0 truncate text-xs font-semibold text-slate-800">
                        {r.requisition || <span className="italic text-slate-400">Untitled req</span>}
                      </span>
                      <span
                    ...
                    <div className="flex items-center gap-2 mt-1 text-[10px] text-slate-500">
                      ...
                      {r.ageing > 0 && (
                        <>
                          ...
                          <span className={r.ageing >= 45 ? 'text-rose-600' : r.ageing >= 21 ? 'text-amber-600' : 'text-slate-500'}>
                            {r.ageing}d
                          </span>
                        </>
                      )}
                      {r.aiProbability > 0 && (
                        <>
                          <span className="text-slate-300">·</span>
                          <span className={r.aiProbability >= 65 ? 'text-emerald-600 font-semibold' : r.aiProbability >= 40 ? 'text-amber-600' : 'text-slate-500'}>
                            {r.aiProbability}% AI
                          </span>
                        </>
                      )}
          ...
          <>
            <header className="pb-3 mb-4 border-b border-slate-100 flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-base font-extrabold text-slate-900 tracking-tight truncate">{selected.account}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Requisition Title</label>
              ...
              <div>
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Positions</label>
              ...
              <div>
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Prob % (manual, blank = AI)</label>
              ...
              <div>
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Close-by Date</label>
                <Input type="date" value={selected.closeByDate || ''} onChange={(e) => onSave(selected.id, 'close_by_date', e.target.value)} className="mt-1" />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Client SPOC</label>
              ...
              <div className="md:col-span-2">
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Anticipation / Notes</label>
              ...
              <div className="md:col-span-2 flex items-center gap-3 pt-2 border-t border-slate-100 text-[11px] text-slate-500 flex-wrap">
                <span>Open: <strong className="text-slate-700">{selected.openPositions}</strong> of {selected.newPositions}</span>
                <span className="text-slate-300">·</span>
                <span>Ageing: <strong className="text-slate-700">{selected.ageing} d</strong></span>
                <span className="text-slate-300">·</span>
                <span>AI Prob: <strong className="text-slate-700">{selected.aiProbability}%</strong></span>
                <span className="text-slate-300">·</span>
                <span>Risk: <strong className="text-slate-700 capitalize">{selected.risk}</strong></span>
              </div>
            </div>
          </>
        )}
