...
- Return ONLY the JSON object.`;
  ...
  try {
    ...
    console.warn('[process-account-document] JSON parse failed, attempting repair. stop_reason=%s err=%s', stopReason, (e as Error).message);
    parsed = repairTruncatedJson(match[0]);
  }
  return { summary: (parsed.summary as string) ?? '', topics: parsed };
}

/** Best-effort repair of JSON that got cut off at max_tokens. Walks the
 *  string tracking bracket + string state, truncates back to the last
 *  complete element, then closes any still-open brackets. */
